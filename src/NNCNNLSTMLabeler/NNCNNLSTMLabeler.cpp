...
int Classifier::createAlphabet(const vector<Instance>& vecInsts) {
	...
	int feat_type_num = vecInsts[0].m_feats.size();
	m_vec_feat_stats.resize(feat_type_num);
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		const Instance *pInstance = &vecInsts[numInstance];

		const vector<string> &feats = pInstance->m_feats;
		const string &label = pInstance->m_label;

		m_driver._modelparams.labelAlpha.from_string(label);
		for (int i = 0; i < feat_type_num; i++)
		{
			m_vec_feat_stats[i][feats[i]]++;
		}
	...
	cout << "Feat type num: " << feat_type_num << endl;
...
void Classifier::extractFeature(Feature& feat, const Instance* pInstance) {
	feat.clear();
	feat.m_feats = pInstance->m_feats;
}
...
void Classifier::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	std::cout << "Training example number: " << trainExamples.size() << std::endl;
	std::cout << "Dev example number: " << devExamples.size() << std::endl;
	std::cout << "Test example number: " << testExamples.size() << std::endl;
	...
	int feat_type_num = m_vec_feat_stats.size();
	m_driver._modelparams.vecFeatAlpha.resize(feat_type_num);
	m_driver._modelparams.vecFeatTable.resize(feat_type_num);
	for (int idx = 0; idx < feat_type_num; idx++) {
		m_vec_feat_stats[idx][unknownkey] = m_options.featCutOff + 1;
		m_driver._modelparams.vecFeatAlpha[idx].initial(m_vec_feat_stats[idx], m_options.featCutOff);
		m_driver._modelparams.vecFeatTable[idx].initial(&m_driver._modelparams.vecFeatAlpha[idx], m_options.featEmbSize, m_options.featEmbFineTune);
	}
